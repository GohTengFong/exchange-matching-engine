...
public:
	...
	void handleCancelOrder(int id, std::unordered_map<int, ClientOrderRecord>& clientsOrders);
	bool handleBuyOrder(std::string instrument, int id, int price, int cnt);
	bool handleSellOrder(std::string instrument, int id, int price, int cnt);

private:
	std::unordered_map<std::string, std::pair<std::pair<std::shared_ptr<SellOrderbook>, std::shared_ptr<BuyOrderbook>>, std::shared_ptr<std::mutex>>> instrumentToBooks;
	std::mutex globalMut;
	std::atomic<int> currTime;
